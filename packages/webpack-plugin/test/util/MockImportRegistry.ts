...
import { ReadonlyOptions as CssBlockOptions, ImportedFile, Importer, PathBasedImporter, Syntax } from "css-blocks";
...
export class MockImporter extends PathBasedImporter {
  ...
  identifier(fromFile: string | null, importPath: string, _options: CssBlockOptions) {
    if (fromFile) {
  ...
  import(resolvedPath: string, options: CssBlockOptions): Promise<ImportedFile> {
